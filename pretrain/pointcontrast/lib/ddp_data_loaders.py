...
class ScanNetMatchPairDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, idx):
    ...
    # print(f"colour0_shape: {color0.shape}, colour1_shape: {color1.shape}")
    ...
    if self.random_scale and random.random() < 0.95:
      ...
      # print(f"\n\nScaling Factor: {matching_search_voxel_size}")
      xyz0 = scale * xyz0
      xyz1 = scale * xyz1
    ...
    if self.random_rotation:
      ...
      # print(f"xyz0_shape: {xyz0.shape}, xyz1_shape: {xyz1.shape}")
      # print("END")
    ...
    # print(sel0[1].shape)
    unique_coords = (xyz0 / self.voxel_size)[sel0[1]]
    # print("Shape of uniq cood is: ", (unique_coords.shape))
    ...
    # print("pcd0_shape is :" , np.asarray(pcd0.points).shape)
    # print("pcd1_shape is :" , np.asarray(pcd1.points).shape)

    # Select features and points using the returned voxelized indices
    # print(type(color0), type(color1))
    # print("len of color0 is :", len(color0), "len of color1 is :", len(color0[0]))
    # print(sel0[1])
    pcd0.colors = o3d.utility.Vector3dVector(color0[sel0[1]])  ##
    pcd1.colors = o3d.utility.Vector3dVector(color1[sel1[1]])  ##

    print("colours")
    print((np.asarray(pcd0.colors)).shape, (np.asarray(pcd1.colors)).shape)

    pcd0.points = o3d.utility.Vector3dVector(np.array(pcd0.points)[sel0[1]]) ##
    pcd1.points = o3d.utility.Vector3dVector(np.array(pcd1.points)[sel1[1]]) ##

    print("points")
    print((np.asarray(pcd0.points)).shape, (np.asarray(pcd1.points)).shape)
