...
class HardestContrastiveLossTrainer(ContrastiveLossTrainer):
  ...
  def _train_iter(self, data_loader_iter, timers):
    ...
    sinput0 = ME.SparseTensor(
        input_dict['sinput0_F'].to(self.cur_device), coordinates=input_dict['sinput0_C'].to(self.cur_device))
    F0 = self.model(sinput0).F

    sinput1 = ME.SparseTensor(
        input_dict['sinput1_F'].to(self.cur_device), coordinates=input_dict['sinput1_C'].to(self.cur_device))
